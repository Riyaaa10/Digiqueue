...
import { ArrowLeft, Users, Play, Pause, SkipForward } from "lucide-react";
import { useToast } from "../../hooks/use-toast";
import { collection, onSnapshot, doc, updateDoc, getDoc, addDoc, Timestamp, query, where, orderBy, getDocs } from "firebase/firestore";
...
export default function LiveQueuePage() {
  ...
  const sendNextUserEmail = async (data) => {
  try {
    await emailjs.send(
      "service_u81x35g",     // 🔁 Same as before
      "template_yx0j67d", // 🔁 This is the new template ID
      data,
      "WO190KU_i5IkE4nG3"
    );
    console.log("✅ Next user email sent");
  } catch (error) {
    console.error("❌ Error sending next user email:", error);
  }
};
  ...
  const markAsCompleted = async (bookingId: string, nextUserId: string | undefined, serviceName: string) => {
  try {
    ...
    if (nextUserId) {
      ...
      if (nextUserSnap.exists()) {
        ...
        // EmailJS - Send email
        await emailjs.send(
          "service_u81x35g", // Replace with your actual service ID
          "template_yx0j67d", // Replace with your actual template ID
          {
            user_name: nextUserData.name || "User",
            email: nextUserData.email,
            org_name: serviceName,
            location: nextUserData.location || "your location",
            date: nextUserData.Date,
            time: nextUserData.Time,
            message: `You're next in the queue for ${serviceName}. Please be ready!`,
            type: "queue-update",
            token: Math.floor(Math.random() * 100 + 1).toString(), // optional OTP
          },
          "WO190KU_i5IkE4nG3" // Replace with your EmailJS public key
        );

        console.log("📧 Email sent to next user:", nextUserData.email);
      } else {
        console.warn("⚠️ Next user document not found.");
      }
    }
  } catch (error) {
    console.error("❌ Failed in markAsCompleted:", error);
  }
};
