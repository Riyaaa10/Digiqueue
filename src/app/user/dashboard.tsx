...
import { Calendar, Clock, MapPin, Plus, Bell, TrendingUp } from "lucide-react";
...
export default function DashboardPage() {
      ...
      const upcoming = bookings
        .map((booking) => {
          ...
          try {
            ...
            let parsedTime = booking.time ? parse(booking.time, "hh:mm a", new Date()) : new Date();
  ...
  // Calculate queue position if there are upcoming bookings
  useEffect(() => {
    if (upcomingBookings.length === 0) return;

    const userBooking = upcomingBookings[0]; // assuming the earliest one
    const allQueueQuery = query(
      collection(db, "bookings"),
      where("service", "==", userBooking.service),
      where("status", "==", "pending"),
      orderBy("date", "asc"),
      orderBy("time", "asc")
    );

    const unsubscribeQueue = onSnapshot(allQueueQuery, (snapshot) => {
      const allBookings = snapshot.docs.map(doc => doc.data());
      const userDateTime = new Date(`${userBooking.date}T${userBooking.time}`);

      const aheadCount = allBookings.filter(b => {
        const bDateTime = new Date(`${b.date}T${b.time}`);
        return bDateTime < userDateTime;
      }).length;

      setQueuePosition(aheadCount);
    });
      ...
      <main className="container py-8 relative z-10">
                  ...
                  <div key={booking.id} className="flex items-center justify-between p-4 border rounded-lg bg-white/50">
                    ...
                    <div className="text-right">
                      <Badge variant="secondary" className="mb-2">
                        Position #{queuePosition !== null ? queuePosition : 'N/A'}
                      </Badge>
        ...
        {queuePosition !== null && (
          <div className="text-sm text-blue-600 font-medium mt-4">
            People ahead of you in {upcomingBookings[0]?.service} queue: {queuePosition}
          </div>
        )}
      </main>
    </div>
